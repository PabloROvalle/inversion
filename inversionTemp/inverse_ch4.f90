program inverse_ch4
  ...
  real, dimension(nlevel) :: p, t, tau_haze, z, grav, dt, sigma, tau_aer
...
!==============================================================================
  ...
  call read_pta(file_pta,p_mol,vmr,p,T,profil, tau_aer)
...
!  ----- Initialisation for inversion
  ...
  do
     i = i + 1
     call coeur_ch4(dt,sigma,p,size(wave),kk,spec_obs,spec_syn, error,A, dr)
     t = t + dt
     call info_content_ch4(p,t,profil,p_mol,lat,mue,fwhm,file_opa,size(wave),wave,spec_syn,kk,tau_aer)
  ...
  call write_inv(file_inv,p,t,sigma)
