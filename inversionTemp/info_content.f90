...
!==============================================Derivee temperature
    ...
    do k = 1, nlevel-1
       ...
       call convol_jwst(size(radiance),radiance,fwhm,fnu,nflux,wave,kk(:,k))
    end do
    deallocate(radiance)
...
20  deallocate(fnu)
  ...
  subroutine coeur_ch4(dt,sigma,p,nflux,kk,spec_obs,spec_syn,error,A,dr)
...
!------------------------------------------------------------------------------
    ...
    dt = alpha * matmul(w,(spec_obs-spec_syn))

    !-------------------------------------------
    open(unit=414, file='KERNEL.txt', ACTION="write")
    do i=1, nlevel
