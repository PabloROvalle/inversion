program jupiter_ch4
  ...
  integer :: i, p_mol,j, n_inv, l, ii, shift, numinv, niter, cloudstate, place
  ...
  real, dimension(nlevel) :: p, t, grav, cloudinv, taucloud
...
!==============================================================================
  ...
  call read_pta(file_pta,p_mol,vmr,p,T,profil,cloudstate,cloudinv,taucloud)
  ...
  if (n_inv == 0) then
    call info_content_ch4(p,t,profil,p_mol,shift,cloudstate,cloudinv,taucloud,lat,mue,fwhm,file_opa,size(wave),wave,spec_syn,n_inv,inv)
  else
    ...
    ochi2 = 1e38
    do
      call info_content_ch4(p,t,profil,p_mol,shift,cloudstate,cloudinv,taucloud,lat,mue,fwhm,file_opa,size(wave),wave,spec_syn,n_inv,inv,kk)
      ...
      ochi2 = chi2
      call coeur_ch4(delta,sigma_delta,A,dr,p,size(wave),cloudstate,n_inv,kk,spec_obs,spec_syn,error)
