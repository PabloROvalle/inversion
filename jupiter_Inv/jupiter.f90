program jupiter_ch4
  ...
  integer :: i, p_mol,j, n_inv, l, ii, shift, numinv, niter
...
!==============================================================================
  call read_fuel(file_fuel,lat,mui,mue,p_mol,vmr,inv,arr_inv,file_opa)
  ...
  if (n_inv == 0) then
    ...
    do
      ...
      if (abs(chi2-ochi2)/chi2 < 0.15) exit
      ochi2 = chi2
...
!======= Write the summary of the inversion: number iterations, chi2 and degrees of freedom
  if (n_inv /= 0) then
    open(129,file='jupiter_ch4.sum',status='unknown',form='formatted')
    write(129,'(I4)')(niter)
  ...
  call write_inv('jupiter_ch4.inv',p,t,profil,sigma_delta, n_inv,inv_pos, vmr)
  if (n_inv /= 0) call write_kernel('jupiter_ch4.avg',n_inv,A)
