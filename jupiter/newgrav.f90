...
!      given by Lindal et al., 1986, Astr. J., 90 (6), 1136-1146, as the
!      expression
!                      ___inf
!          G M  (    \                    (R)**2n                  )
...
!-------------------------------------------------------------------------------
    ...
    subroutine newgrav (lat_in, h, g) 
      ...
      use declaration, only : RJup, J2Jup, J3Jup, J4Jup, omega, ellip,MassJup, Grav, pi
      implicit none

      real, intent(in) :: lat_in, h ! Latitude (degrees), Height above reference surface (km)
      real, intent(out)::  g	!gravity acc. (m/s2)
      integer :: I
      real :: lat, latc, thetagc, slatc, r, legpol, clatc, gtheta, Rr, radius
